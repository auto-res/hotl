...
def VALUE_init():
    ###値の初期値########################
    st.session_state.txt_OBJECTIVE_value ="""The objective is to increase Accuracy in classification problems."""
    st.session_state.txt_M_pre_name_value ="""CNN with Dropout layer"""

    st.session_state.txt_M_pre_code_value ="""class Net(nn.Module):
    def __init__(self):
    ...
    st.session_state.txt_M_patch_name_value ="""CNN with MuDropout layer"""

    st.session_state.txt_M_patch_code_value ="""class Net(nn.Module):
    def __init__(self):
        super(Net, self).__init__()
        self.conv1 = nn.Conv2d(3, 6, 5)
        self.pool = nn.MaxPool2d(2, 2)
    ...
    st.session_state.reference_python_code_value ="""import numpy as np
import torch
